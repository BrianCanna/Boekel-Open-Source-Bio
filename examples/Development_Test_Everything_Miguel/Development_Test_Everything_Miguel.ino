...
void example_draw_common()
{
  ...
  // draw some text
  osb_drawtext(10, 10, "Boekel Open Source Bio");
  osb_drawtext(10, 30, "Temperature: ");
  Serial.print("Temperature: ");

  if(osb_get_temperaturevalid())
  {
    dtostrf(osb_get_temperature(), 3, 1, buffer);
    osb_drawtext(160, 30, buffer);
  ...
  osb_setforecolor(OSB_COLOR_BLUE);
  osb_drawgraph_bars(1, 100, 318, 110, 255, &data[0]);
  ...
  osb_drawgraph_step(1, 100, 318, 110, 16, &data[0]);
...
void example_draw_pH()
{
  ...
  // draw some text
  osb_drawtext(10, 50, "pH Readings");
  Serial.println("pH");

  osb_drawtext(10, 70, "pH");
  osb_drawtext(10, 90, "Approx mV");

  if(osb_get_readingvalid())
  {
    dtostrf(osb_get_ph(), 2, 3, buffer);
    osb_drawtext(160, 70, buffer);
    Serial.print("pH: ");
    Serial.println(buffer);
    dtostrf(osb_get_ph_mV(), 3, 1, buffer);
    osb_drawtext(160, 90, buffer);
...
void example_draw_do()
{
  ...
  // draw some text
  osb_drawtext(10, 50, "Dissolved Oxygen Readings");
  Serial.println("Dissolved Oxygen");

  osb_drawtext(10, 70, "mg/L");
  osb_drawtext(10, 90, "% Saturation");

  if(osb_get_readingvalid())
  {
    dtostrf(osb_get_do_mgl(), 3, 1, buffer);
    osb_drawtext(160, 70, buffer);
    Serial.print("mg/L: ");
    Serial.println(buffer);
    dtostrf(osb_get_do_percentage(), 3, 1, buffer);
    osb_drawtext(160, 90, buffer);
...
void example_draw_ec()
{
  ...
  // draw some text
  osb_drawtext(10, 50, "Electrolytic Conductivity");
  Serial.println("Electrolytic Conductivity");

  osb_drawtext(10, 70, "uS/cm");
  osb_drawtext(10, 90, "PSS");
  //osb_drawtext(10, 160, "TDS");

  if(osb_get_readingvalid())
  {
    dtostrf(osb_get_ec(), 6, 1, buffer);
    osb_drawtext(160, 70, buffer);
    Serial.print("uS/cm: ");
    Serial.println(buffer);
    dtostrf(osb_get_ec_pss(), 2, 1, buffer);
    osb_drawtext(160, 90, buffer);
    Serial.print("PSS: ");
    Serial.println(buffer);
    dtostrf(osb_get_ec_tds(), 2, 1, buffer);
    //osb_drawtext(160, 160, buffer);
    Serial.print("TDS: ");
    Serial.println(buffer);
  }
  else
...
void loop()
{
  ...
  Serial.print(osb_get_readingtype());
  Serial.println();

  switch(osb_get_readingtype())
  {

  case OSB_READING_TYPE_PH:
    example_draw_pH();
    break;

  case OSB_READING_TYPE_EC:
    example_draw_ec();
    break;

  case OSB_READING_TYPE_DO:
    example_draw_do();
    break;

  case OSB_READING_TYPE_TEMPERATURE:
    example_draw_temperature();
    break;
